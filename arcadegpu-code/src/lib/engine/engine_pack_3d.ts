...
import { EnginePack3DBlenderWebSocket } from './engine_pack_3d_blender_ws';
...
class EnginePack3D {
  ...
  blenderWS: EnginePack3DBlenderWebSocket;
  ...
  constructor() {
    ...
    this.blenderWS = new EnginePack3DBlenderWebSocket({
      onOpen: () => this.#handleOpenBlenderWebSocket(),
      onClose: () => this.#handleCloseBlenderWebSocket(),
      onCameraChange: (data: any) => this.#handleCameraChangeBlenderWebSocket(data),
    });
  }
  ...
  static async createFromFile(path: string): Promise<EnginePack3D> {
    ...
    for (const entry of zip.file(/.*/)) {
      ...
      if (file != null && infos.name == 'camera') {
        ...
        pack.cameraPosition = data['Position'];
        pack.cameraRotation = data['Rotation'];
        pack.cameraMatrix = data['Matrix'];
        pack.cameraFovy = data['Fovy'];
        pack.cameraNear = data['Near'];
        pack.cameraFar = data['Far'];
        pack.camera.setPosition(pack.cameraPosition[0], pack.cameraPosition[1], pack.cameraPosition[2]);
        pack.camera.setRotation(pack.cameraRotation[0], pack.cameraRotation[1], pack.cameraRotation[2]);
        pack.camera.setPerspectiveFovy(pack.cameraFovy);
        pack.camera.setPerspectiveNear(pack.cameraNear);
        pack.camera.setPerspectiveFar(pack.cameraFar);
      }
  ...
  enableBlenderConnexion() {
    this.blenderWS.open();
  }

  disableBlenderConnexion() {
    this.blenderWS.close();
  }

  #handleOpenBlenderWebSocket() {
    const title = document.getElementById('TITLE');
    if (title) {
      title.innerHTML = 'Blender → Game Engine: <span style="color:cyan">Link Established</span>';
    }
  }

  #handleCloseBlenderWebSocket() {
    const title = document.getElementById('TITLE');
    if (title) {
      title.innerHTML = 'Blender → Game Engine: <span style="color:orangered">No Active Link</span>';
    }
  }

  #handleCameraChangeBlenderWebSocket(data: any) {
    this.camera.setPosition(data['Position'][0], data['Position'][1], data['Position'][2]);
    this.camera.setRotation(data['Rotation'][0], data['Rotation'][1], data['Rotation'][2]);
    this.camera.setPerspectiveFovy(data['Fovy']);
    this.camera.setPerspectiveNear(data['Near']);
    this.camera.setPerspectiveFar(data['Far']);
  }
}
