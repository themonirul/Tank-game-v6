...
struct MaterialParams {
  ...
  LIGHT_GROUP: f32,
  DECAL_GROUP: f32,
  JITTER_VERTEX_ENABLED: f32,
  JITTER_VERTEX_LEVEL: f32,
  GOURAUD_SHADING_ENABLED: f32,
  ARCADE_ENABLED: f32,
  ARCADE_START_COLOR_R: f32,
  ARCADE_START_COLOR_G: f32,
  ARCADE_START_COLOR_B: f32,
  ARCADE_END_COLOR_R: f32,
  ARCADE_END_COLOR_G: f32,
  ARCADE_END_COLOR_B: f32,
  ARCADE_SHARP_COLOR_R: f32,
  ARCADE_SHARP_COLOR_G: f32,
  ARCADE_SHARP_COLOR_B: f32,
  DISPLACE_TEXTURE: f32,
  DISPLACE_SECONDARY_TEXTURE: f32,
  DISPLACE_NORM_TEXTURE: f32,
  DISPLACE_DISSOLVE_TEXTURE: f32,
  ${data.MAT_S00}: f32,
  ${data.MAT_S01}: f32,
  ${data.MAT_S02}: f32,
  ${data.MAT_S03}: f32,
...
fn main(
  ...
  if (MAT_PARAMS.GOURAUD_SHADING_ENABLED == 1.0)
  {
    ...
    for (var i: u32 = 0u; i < u32(SCENE_INFOS.POINT_LIGHT_COUNT); i++)
    {
      if (POINT_LIGHTS[i].GROUP == 0.0 || POINT_LIGHTS[i].GROUP == MAT_PARAMS.LIGHT_GROUP)
      {
        ...
        gauraudColor += diffuseFactor * POINT_LIGHTS[i].DIFFUSE * POINT_LIGHTS[i].INTENSITY * atten;
      }
    }
  }
  ...
  if (MAT_PARAMS.JITTER_VERTEX_ENABLED == 1.0)
  {
    var x = output.Position.x / output.Position.w;
    var y = output.Position.y / output.Position.w;
    x = floor(x * MAT_PARAMS.JITTER_VERTEX_LEVEL) / MAT_PARAMS.JITTER_VERTEX_LEVEL;
    y = floor(y * MAT_PARAMS.JITTER_VERTEX_LEVEL) / MAT_PARAMS.JITTER_VERTEX_LEVEL;
    output.Position.x = x * output.Position.w;
    output.Position.y = y * output.Position.w;
  }
...
fn main(
  ...
  var normTextureUV = fragUV;
  var viewDirWorld = normalize(fragPos - cameraPos);
  var displaceUV = vec2(0.0, 0.0);

  if (MAT_PARAMS.HAS_DISPLACEMENT_MAP == 1.0)
  {
    displaceUV = CalcDisplacementMap(displacementTextureUV);
    textureUV += displaceUV * MAT_PARAMS.DISPLACE_TEXTURE;
    secondaryTextureUV += displaceUV * MAT_PARAMS.DISPLACE_SECONDARY_TEXTURE;
    normTextureUV += displaceUV * MAT_PARAMS.DISPLACE_NORM_TEXTURE;
    dissolveTextureUV += displaceUV * MAT_PARAMS.DISPLACE_DISSOLVE_TEXTURE;
  }
  ...
  if (MAT_PARAMS.BLEND_COLOR_MODE == 2.0)
  {
    texel += MAT_COLORS.BLEND;
  }
  ...
  {
    fragNormal = CalcNormalMap(fragNormal, fragTangent, fragBinormal, normTextureUV);
  }
  ...
  else if (MAT_PARAMS.GOURAUD_SHADING_ENABLED == 1.0)
  {
  ...
  {
    var envColor = CalcEnvMap(fragNormal, fragPos, cameraPos);
    outputColor = mix(outputColor, envColor, envColor.a);
  }

  if (MAT_PARAMS.ARCADE_ENABLED == 1.0)
  {
    var frontFactor = min(dot(fragNormal, viewDirWorld), 0.0);
    var sharpFactor = max(dot(fragNormal, viewDirWorld), 0.0);
    var a = textureSample(MAT_NORM_TEXTURE, MAT_NORM_SAMPLER, textureUV).a;

    var startColor = vec3(MAT_PARAMS.ARCADE_START_COLOR_R, MAT_PARAMS.ARCADE_START_COLOR_G, MAT_PARAMS.ARCADE_START_COLOR_B);
    var endColor = vec3(MAT_PARAMS.ARCADE_END_COLOR_R, MAT_PARAMS.ARCADE_END_COLOR_G, MAT_PARAMS.ARCADE_END_COLOR_B);

    var sharpColor = vec3(MAT_PARAMS.ARCADE_SHARP_COLOR_R, MAT_PARAMS.ARCADE_SHARP_COLOR_G, MAT_PARAMS.ARCADE_SHARP_COLOR_B) * sharpFactor;
    var frontColor = mix(startColor, endColor, abs(frontFactor));

    outputColor = mix(outputColor, vec4(frontColor + sharpColor, 1.0), a);
  }
  ...
  var flags = u32(MESH_INFOS.TAG.b);

  ${data.FRAG_BEGIN}
  var output: FragOutput;
  output.Normal = vec4(normalize(fragNormal), 1.0);
  output.Tag = MESH_INFOS.TAG;
...
{
  var normalPixel = textureSample(MAT_NORM_TEXTURE, MAT_NORM_SAMPLER, textureUV);
  var normalMapVect = normalPixel.xyz * 2.0 - 1.0;
  let finalNormalVect = vec3(
    normalMapVect.x * MAT_PARAMS.NORMAL_INTENSITY,
    normalMapVect.y * MAT_PARAMS.NORMAL_INTENSITY,
    normalMapVect.z
  );
...
fn CalcLights(normal: vec3<f32>, fragPos: vec3<f32>, cameraPos: vec3<f32>, textureUV: vec2<f32>, shadow: f32) -> vec4<f32>
{
  ...
  var globalAmbient = vec3(SCENE_INFOS.AMBIENT_COLOR_R, SCENE_INFOS.AMBIENT_COLOR_G, SCENE_INFOS.AMBIENT_COLOR_B);
  var isMaterialAmbientEmpty = all(MAT_COLORS.AMBIENT <= vec3f(0.0001));
  var finalAmbient = select(MAT_COLORS.AMBIENT, globalAmbient, isMaterialAmbientEmpty);
  return totalLight + vec4(finalAmbient, 1.0);
}
