...
interface MATOptions {
  ...
  jitterVertexEnabled?: number;
  jitterVertexLevel?: number;
  gouraudShadingEnabled?: boolean;
  arcadeEnabled?: boolean;
  arcadeStartColor?: vec3;
  arcadeEndColor?: vec3;
  arcadeSharpColor?: vec3;
  displaceTexture?: boolean;
  displaceSecondaryTexture?: boolean;
  displaceNormTexture?: boolean;
  displaceDissolveTexture?: boolean;
...
class Gfx3Material {
  ...
  constructor(options: MATOptions) {
    ...
    this.colors[15] = 0.0;
    this.colors[16] = options.blendColor ? options.blendColor[0] : 1.0;
    this.colors[17] = options.blendColor ? options.blendColor[1] : 1.0;
    this.colors[18] = options.blendColor ? options.blendColor[2] : 1.0;
    this.colors[19] = options.blendColor ? options.blendColor[3] : 1.0;
    ...
    this.params = this.grp2.setFloat(1, 'MAT_PARAMS', 62); 
    ...
    this.params[4] = options.secondaryTexture ? (options.secondaryTextureBlendMode == 'MUL' ? 1.0 : 2.0) : 0.0; // MUL = 1, MIX = 2
    ...
    this.params[26] = options.blendColorMode ? (options.blendColorMode == 'MUL' ? 1.0 : 2.0) : 0.0; // MUL = 1, ADD = 2
    this.params[27] = options.lightGroup ?? 0;
    this.params[28] = options.decalGroup ?? 0;
    this.params[29] = options.jitterVertexEnabled ?? 0;
    this.params[30] = options.jitterVertexLevel ?? 120.0;
    this.params[31] = options.gouraudShadingEnabled ? 1.0 : 0.0;
    this.params[32] = options.arcadeEnabled ? 1.0 : 0.0;
    this.params[33] = options.arcadeStartColor ? options.arcadeStartColor[0] : 0.0;
    this.params[34] = options.arcadeStartColor ? options.arcadeStartColor[1] : 0.0;
    this.params[35] = options.arcadeStartColor ? options.arcadeStartColor[2] : 0.0;
    this.params[36] = options.arcadeEndColor ? options.arcadeEndColor[0] : 0.0;
    this.params[37] = options.arcadeEndColor ? options.arcadeEndColor[1] : 0.0;
    this.params[38] = options.arcadeEndColor ? options.arcadeEndColor[2] : 0.0;
    this.params[39] = options.arcadeSharpColor ? options.arcadeSharpColor[0] : 0.0;
    this.params[40] = options.arcadeSharpColor ? options.arcadeSharpColor[1] : 0.0;
    this.params[41] = options.arcadeSharpColor ? options.arcadeSharpColor[2] : 0.0;
    this.params[42] = options.displaceTexture ? 1.0 : 0.0;
    this.params[43] = options.displaceSecondaryTexture ? 1.0 : 0.0;
    this.params[44] = options.displaceNormTexture ? 1.0 : 0.0;
    this.params[45] = options.displaceDissolveTexture ? 1.0 : 0.0;
    if (options.sParams) {
      for (const sParam of options.sParams) {
        const paramIndex = Object.values(MAT_PARAMS_VARS).findIndex(n => n == sParam.name);
        if (paramIndex != -1) {
          this.params[46 + paramIndex] = sParam.value ?? 0.0;
        }
      }
    }
  ...
  static async createFromFile(path: string, textureDir: string = ''): Promise<Gfx3Material> {
    ...
    return new Gfx3Material({
      ...
      blendColorMode: json['BlendColorMode'],
      ...
      gouraudShadingEnabled: json['GouraudShadingEnabled'],
      arcadeEnabled: json['ArcadeEnabled'],
      arcadeStartColor: json['ArcadeStartColor'],
      arcadeEndColor: json['ArcadeEndColor'],
      arcadeSharpColor: json['ArcadeSharpColor'],
      displaceTexture: json['DisplaceTexture'],
      displaceSecondaryTexture: json['DisplaceSecondaryTexture'],
      displaceNormTexture: json['DisplaceNormTexture'],
      displaceDissolveTexture: json['DisplaceDissolveTexture'],
      sParams: sParams
    });
  }
  ...
  /**
   * Enable jitter vertex.
   * 
   * @param {boolean} enabled - The jitter vertex enabled flag.
   */
  setJitterVertexEnabled(enabled: boolean): void {
    this.params[29] = enabled ? 1.0 : 0.0;
    this.dataChanged = true;
  }

  /**
   * Set the jitter vertex level.
   * 
   * @param {number} level - The jitter vertex level.
   */
  setJitterVertexLevel(level: number): void {
    this.params[30] = level;
    this.dataChanged = true;
  }

  /**
   * Enable gouraud shading.
   * Work only with diffuse color of lights.
   * 
   * @param {boolean} enabled - The gouraud shading enabled flag.
   */
  setGouraudShadingEnabled(enabled: boolean): void {
    this.params[31] = enabled ? 1.0 : 0.0;
    this.dataChanged = true;
  }

  /**
   * Enable arcade effect.
   * 
   * @param {boolean} enabled - The arcade effect enabled flag.
   */
  setArcadeEnabled(enabled: boolean): void {
    this.params[32] = enabled ? 1.0 : 0.0;
    this.dataChanged = true;
  }

  /**
   * Set the arcade color when pixel is facing the camera (start color).
   * 
   * @param {number} r - The red component.
   * @param {number} g - The green component.
   * @param {number} b - The blue component.
   */
  setArcadeStartColor(r: number, g: number, b: number): void {
    this.params[33] = r;
    this.params[34] = g;
    this.params[35] = b;
    this.dataChanged = true;
  ...
  /**
   * Set the arcade color when pixel is facing the camera (end color).
   * 
   * @param {number} r - The red component.
   * @param {number} g - The green component.
   * @param {number} b - The blue component.
   */
  setArcadeEndColor(r: number, g: number, b: number): void {
    this.params[36] = r;
    this.params[37] = g;
    this.params[38] = b;
    this.dataChanged = true;
  ...
  /**
   * Set the arcade sharp color.
   * 
   * @param {number} r - The red component.
   * @param {number} g - The green component.
   * @param {number} b - The blue component.
   */
  setArcadeSharpColor(r: number, g: number, b: number): void {
    this.params[39] = r;
    this.params[40] = g;
    this.params[41] = b;
    this.dataChanged = true;
  ...
  /**
   * Enable displace environment map.
   * 
   * @param {boolean} enabled - The displace environment map enabled flag.
   */
  displaceTexture(enabled: boolean): void {
    this.params[42] = enabled ? 1.0 : 0.0;
    this.dataChanged = true;
  }

  /**
   * Enable displace secondary texture.
   * 
   * @param {boolean} enabled - The displace secondary texture enabled flag.
   */
  displaceSecondayTexture(enabled: boolean): void {
    this.params[43] = enabled ? 1.0 : 0.0;
    this.dataChanged = true;
  }

  /**
   * Enable displace normal map.
   * 
   * @param {boolean} enabled - The displace normal map enabled flag.
   */
  displaceNormTexture(enabled: boolean): void {
    this.params[44] = enabled ? 1.0 : 0.0;
    this.dataChanged = true;
  }

  /**
   * Enable displace dissolve map.
   * 
   * @param {boolean} enabled - The displace dissolve map enabled flag.
   */
  displaceDissolveTexture(enabled: boolean): void {
    this.params[45] = enabled ? 1.0 : 0.0;
    this.dataChanged = true;
  }
  ...
  setCustomParam(name: string, value: number): void {
    ...
    this.params[46 + paramIndex] = value;
  }
  ...
  getCustomParam(name: string): number {
    ...
    return this.params[46 + paramIndex];
  }
