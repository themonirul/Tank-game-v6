  ...
  async #handlePointerDown(e: PointerEvent): Promise<void> {
    if (this.pointerLockEnabled && !document.pointerLockElement) {
      try {
        ...
        // Ignore pointer lock errors quietly
      }
    }

    const rect = this.container.getBoundingClientRect();
    const leftR = e.clientX - rect.left;
    const topR = e.clientY - rect.top;
    if (leftR < 0 || leftR > coreManager.getWidth() || topR < 0 || topR > coreManager.getHeight()) {
      return;
    }
